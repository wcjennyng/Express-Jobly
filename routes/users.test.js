...
describe('POST /users/:username/jobs/:id', function () {
  test('works for admin', async function() {
    const res = await request(app).post(`/users/u1/jobs/${testJobIds[1]}`)
                .set("authorization", `Bearer ${adminToken}`)
  ...
  test('unauthorized for non-admin or non-user', async function() {
    const res = await request(app).post(`/users/u1/jobs/${testJobIds[1]}`)
    expect(res.statusCode).toEqual(401)
  })

  test('username not found', async function() {
    const res = await request(app).post(`/users/notfound/jobs/${testJobIds[1]}`)
                .set("authorization", `Bearer ${adminToken}`);
    expect(res.statusCode).toEqual(404)
  })

  test('job not found', async function() {
    const res = await request(app).post(`/users/u1/jobs/0`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(res.statusCode).toEqual(404)
  })
})